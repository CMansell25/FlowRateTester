...
class State:
    ...
    def read_scale_weight(self):
        raw = self.pins.scalePin.read_u16()
        v_ref = 3.3
        r_sense = 150
...
class testState(State):
    ...
    def run(self) -> None:
        ...
        while self.pins.lowfillPin.value() == 1: #consider adding check for high fill pin too to make more fault tolerant
            ...
            self.display.line_out(self.pins.lowfillPin, line = 1)
            self.data = self.updateData(self.data)
            time.sleep_ms(10)
            #self.writeToUSB(data)
    ...
    def updateData(self,data):
        weight = self.read_scale_weight()
        print(f"Weight: {weight} kg")
        with open("data.txt", "a") as db:
            db.write(f"{self.getTimer():.1f},{weight}")
        data = data.append([self.getTimer(), 
                    self.pins.highfillPin,
                    self.pins.lowfillPin,
                    self.pins.relayPin ])
        self.txtWrite(data)
        return data
